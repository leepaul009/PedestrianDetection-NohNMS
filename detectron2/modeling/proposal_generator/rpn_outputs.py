...
from detectron2.structures import Boxes, Instances, pairwise_iou, pairwise_ioa, calculate_iog
...
def find_top_rpn_proposals_all_level(
    proposals,
    pred_objectness_logits,
    images,
    nms_thresh,
    ...
    for bid in range(num_images):
        ...
        keep = ((batch_proposals[:, 2] - batch_proposals[:, 0]) > min_box_side_len) & (
            (batch_proposals[:, 3] - batch_proposals[:, 1]) > min_box_side_len
        )

        batch_proposals = batch_proposals[keep]
        batch_scores = batch_scores[keep]

        num_proposals = min(pre_nms_topk, batch_proposals.shape[0])
        batch_scores, idx = batch_scores.sort(descending=True)

        batch_scores = batch_scores[:num_proposals]
        topk_idx = idx[:num_proposals].flatten()
        batch_proposals = batch_proposals[topk_idx]

        valid_mask = torch.isfinite(batch_proposals).all(dim=1) & torch.isfinite(batch_scores)
        if not valid_mask.all():
            batch_proposals = batch_proposals[valid_mask]
            batch_scores = batch_scores[valid_mask]

        keep = nms(batch_proposals, batch_scores, nms_thresh)
        batch_proposals = Boxes(batch_proposals[keep])
        batch_scores = batch_scores[keep]

        num_proposals = min(post_nms_topk, len(batch_proposals))
        batch_proposals = batch_proposals[:num_proposals]
        batch_scores = batch_scores[:num_proposals]

        res = Instances(image_sizes[bid])
        res.proposal_boxes = batch_proposals
        res.objectness_logits = batch_scores
        results.append(res)
    return results
...
class RPNOutputs(object):
    def __init__(
        ...
        gt_boxes=None,
        smooth_l1_beta=0.0,
        ignore_gt_boxes=None,
        ignore_ioa=False,
        get_gt_per_level=False,
        ...
        Args:
            ...
            smooth_l1_beta (float): The transition point between L1 and L2 loss in
                the smooth L1 loss function. When set to 0, the loss becomes L1. When
                set to +inf, the loss becomes constant 0.
            ignore_gt_boxes (list[Boxes], optional): A list of M elements. 
        """
        ...
        self.ignore_gt_boxes = ignore_gt_boxes
        ...
        self.get_gt_per_level = get_gt_per_level
        self.ignore_ambiguous_sample = ignore_ambiguous_sample
